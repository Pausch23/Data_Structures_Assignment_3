...
# Assignment: 3 - Part 3
# Due Date: 2/9/2026
# Description: Queue class using the static array
# structure. Class has methods for enqueue, dequeue,
# and front.
...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        ...
        :return:        None
        """
        # Preserve old array and create new one once new size exceeds current
        if self._current_size + 1 > self._sa.length():
            curr_arr = self._sa
            self._sa = StaticArray(self._current_size + 1)

            # Loop through each in the curr array and add to new array
            for index in range(self._sa.length() - 1):
                if index <= curr_arr.length():
                    self._sa.set(index, curr_arr.get(index))

        # Set the new size and increment
    ...
    def dequeue(self) -> object:
        ...
        # Set new array to one less length of current
        self._sa = StaticArray(curr_arr.length() - 1)

        # Loop and set items in new list at specific index
        for index in range(self._sa.length()):
            if index == 0:
                new_index = index + 1
                self._sa.set(index, curr_arr.get(new_index))
                # self._current_size -= 1
            else:
                self._sa.set(index, curr_arr.get(index + 1))
